...
class NNPPOPolicy(object):
    ...
    def multi_state_policy(self, states, agent_indices):
        ...
        states_array = []
        for (state_tuple, id) in zip(states, agent_indices):
            states_array.append(state_tuple[:][id])

        states_array = np.array(states_array)

        action_prob_logits = self.network.predict_action_prob_logits(states_array)
