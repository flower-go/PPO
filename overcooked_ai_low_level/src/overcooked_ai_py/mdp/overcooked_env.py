...
class Overcooked(gym.Env):
    ...
    def custom_init_parallel(
        ...
        self.base_env = base_env
        self.featurize_fn = featurize_fn
        # It requires paralel observation space
        self.observation_space = self._setup_parallel_observation_space()
        # self.action_space = gym.spaces.Tuple([gym.spaces.Discrete(len(Action.ALL_ACTIONS))] * 2)
        # But it still requires single action space
        self.action_space = gym.spaces.Discrete(len(Action.ALL_ACTIONS))
        self.reset()

    def _setup_parallel_observation_space(self):
        dummy_mdp = self.base_env.mdp
        dummy_state = dummy_mdp.get_standard_start_state()
        obs_shape = (2,self.featurize_fn(dummy_mdp, dummy_state)[0].shape[0])
    ...
    def _setup_observation_space(self):
        ...
        dummy_state = dummy_mdp.get_standard_start_state()
        # print(dummy_state)
        # obs_shape = self.featurize_fn(dummy_state, self.base_env.mlam)[0].shape
    ...
    def step(self, action):
        ...
        env_info["both_agent_obs"] = both_agents_ob
        env_info["overcooked_state"] = next_state
        env_info["other_agent_env_idx"] = 1 - self.agent_idx

        return both_agents_ob, reward, done, False, env_info # Added Truncated = False for AsyncVectorCompatibility
...
class RewardShapingEnv(SubprocVecEnv):
    """
    Wrapper for the vectorized environment, which
